...
//                    serverState.messageReceived(msg.getData().getString(MESSAGE));
                    serverState.messageReceived(msg.getData().getByteArray(MESSAGE));
                    break;
                case 5:
...
//            objectSerializationCodecFactory.setDecoderMaxObjectSize(Integer.MAX_VALUE);// 设定后服务器可以接收大数据
...
//            acceptor.getSessionConfig().setIdleTime(IdleStatus.BOTH_IDLE, 10);
            ...
            acceptor.setHandler(new MinaServerHandler());
            acceptor.getSessionConfig().setReadBufferSize(460800);
            //10秒无操作释放Session
            acceptor.getSessionConfig().setIdleTime(IdleStatus.BOTH_IDLE, 10);
            try {
