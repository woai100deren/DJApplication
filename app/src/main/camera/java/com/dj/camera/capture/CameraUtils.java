...
public class CameraUtils {
    ...
    public static List<Size> translateCamera1Size(List<Camera.Size> list) {
        List<Size> result = new ArrayList<>();
        for (Camera.Size item : list) {
            result.add(new Size(item.width, item.height));
        }
        return result;
    }

    @SuppressLint("NewApi")
    public static List<Size> translateCamera2Size(android.util.Size[] list) {
        List<Size> result = new ArrayList<>();
        for (android.util.Size item : list) {
            result.add(new Size(item.getWidth(), item.getHeight()));
        }
        return result;
    }

    public static Size choosePreviewSize(List<Size> list, BaseBuilder config) {
        int weight;
        ...
        return new Size(wantWidth, wantHeight);
    }
